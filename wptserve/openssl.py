...
class OpenSSL(object):
    ...
    def __call__(self, cmd, *args, **kwargs):
        ...
        if cmd not in ["x509", "pkcs12"]:
            self.cmd += ["-config", self.conf_path]
        self.cmd += list(args)

        self.logger.debug("Executing openssl command: %s" % " ".join(self.cmd))
...
class OpenSSLEnvironment(object):
    ...
    def __init__(self, logger, openssl_binary="openssl", base_path=None,
                 ...
                 duration=30, base_conf_path=None, install_certs=False):
        ...
        :param install_certs: Attempt to install generated certs into the machine store.
        """
        self.logger = logger
    ...
    def _generate_ca(self):
        ...
        with self._config_openssl(None) as openssl:
            ...
            if self.install_certs:
                self._install_cert(openssl, key_path, cert_path, is_ca=True)
    ...
    def _generate_host_cert(self, hosts):
        ...
        with self._config_openssl(hosts) as openssl:
            ...
            if self.install_certs:
                self._install_cert(openssl, key_path, cert_path, is_ca=False)

        os.unlink(req_path)

        return key_path, cert_path

    def _install_cert(self, openssl, key_path, cert_path, is_ca=False):
        self.logger.info("Installing cert %s" % cert_path)

        if os.name is "nt":
            pfx_path = "%s.pfx" % os.path.splitext(cert_path)[0]

            # convert the .pem cert to .pfx for Windows installation
            openssl("pkcs12",
                    "-inkey", key_path,
                    "-in", cert_path,
                    "-passin", "pass:%s" % self.password,
            ...
            # use certutil.exe to import the .pfx
            cmd = []
            cmd += ["certutil", "-importpfx", "-f", "-p", self.password, "-enterprise"]
            cmd += ["root"] if is_ca else ["my"]
            cmd += [pfx_path]
            self._execute_and_wait(cmd)
        else:
            raise EnvironmentError(
                "don't know how to install auto-generated SSL cert, environment %s not supported" % os.name)

    def _execute_and_wait(self, cmd):
        self.logger.debug("Executing os command: %s" % " ".join(cmd))
